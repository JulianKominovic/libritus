...
function TreeView({ containerRef }: { containerRef: React.RefObject<HTMLDivElement | null> }) {
  ...
  const treeData: NodeModel<CategoryTreeItem | PdfTreeItem | { type: 'C' } | null>[] =
    useMemo(() => {
      ...
      const mappedPdfs: NodeModel<PdfTreeItem>[] = categories.flatMap((category) =>
        category.pdfs.map(
          (pdf) =>
            ({
              id: pdf.id,
          ...
          <ContextMenu>
            ...
            <ContextMenuContent className="w-fit max-w-sm">
              <div className="p-2">
                ...
                <small className="text-morphing-700 pl-6 block">{categoryNode.description}</small>
              </div>
